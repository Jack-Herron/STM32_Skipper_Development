...
#include <stdlib.h>                      // Include C standard library for memory allocation functions
#include <stdint.h>                      // Include C library for fixed-width integer types
#include <stm32f4xx.h>                   // Include MCU-specific definitions for STM32F4xx series
#include <stdbool.h>                     // Include C library for boolean type

#include "../Inc/USB_Host.h"             // Include USB host definitions
#include "../../Config/USB_Host_Config.h"// Include USB host configuration settings
#include "../Inc/USB_Host_Device_Manager.h" // Include USB host device manager definitions

// Static array of port structures for managing USB ports
static USB_Host_Device_Manager___Port_TypeDef USB_Host_Device_Manager___Port[USB_Host___NUMBER_OF_PORTS] = {0};
    ...
    // Static pool of device structures for device allocation
    USB_Host_Device_Manager___Device_TypeDef device_Pool[USB_Host_Device_Manager___DEVICE_POOL_SIZE] = {0};

    // Function to allocate a device from the static pool
    USB_Host_Device_Manager___Device_TypeDef* USB_Host_Device_Manager___Allocate_Device()
    {
        // Iterate through the device pool to find an unallocated device
        for(uint16_t i = 0; i < USB_Host_Device_Manager___DEVICE_POOL_SIZE; i++)
        {
            if(device_Pool[i].is_Allocated == false)
            {
                device_Pool[i].is_Allocated = true;  // Mark the device as allocated
                return(&device_Pool[i]);             // Return pointer to the allocated device
            }
        }
        return(NULL);  // Return NULL if no unallocated device is found
    }

    // Function to free a device back to the static pool
    void USB_Host_Device_Manager___Free_Device(USB_Host_Device_Manager___Device_TypeDef* p_Device)
    {
        p_Device->is_Allocated = false;  // Mark the device as unallocated
    }

#else

    // Function to allocate a device dynamically using malloc
    USB_Host_Device_Manager___Device_TypeDef* USB_Host_Device_Manager___Allocate_Device()
    {
        ...
        if(p_Device != NULL)
        {
            p_Device->is_Allocated = true;  // Mark the device as allocated
        }
        return(p_Device);  // Return pointer to the allocated device or NULL if allocation failed
    }

    // Function to free a dynamically allocated device
    void USB_Host_Device_Manager___Free_Device(USB_Host_Device_Manager___Device_TypeDef* p_Device)
    {
        free(p_Device);  // Free the memory allocated for the device
    }

#endif // USB_Host_Config___DYNAMICALLY_ALLOCATE_DEVICES

// Polling device list for managing periodic device operations
USB_Host_Device_Manager___Polling_Device_List_Typedef USB_Host_Device_Manager___Polling_List[2] = {0};

// Function to add a device to the polling list
...
{
    // Get the polling node associated with the device
    ...
    {
        // Initialize the polling node parameters
        ...
        // Add the polling node to the polling list
        if(USB_Host_Device_Manager___Polling_List[port_Number].first_Node != NULL)
        {
...
// Function to remove a device from the polling list
void USB_Host_Device___Remove_Polling_Device(uint8_t port_Number, uint8_t device_Address)
{
    ...
    if(p_Polling_Node != NULL)
    {
        // Disable the polling node and reset its parameters
        ...
        // Update the polling list to remove the node
...
// Function to add a disconnection callback for a device
...
// Function to get a pointer to a device structure
...
// Function to check if a port is open (no device connected at address 0)
...
// Function to get the device class from the device descriptor
...
// Function to get a pointer to a device's configuration descriptor
...
// Function to get a pointer to an interface descriptor within a configuration
...
// Function to get a pointer to an endpoint descriptor within an interface
...
// Function to get the device descriptor buffer
...
// Function to get the list of language IDs supported by the device
...
// Function to get the manufacturer string descriptor
...
// Function to get the product string descriptor
...
// Function to get the serial number string descriptor
...
// Function to get the length of the serial number string
...
// Function to get the length of the manufacturer string
...
// Function to get the length of the product string
...
// Function to get the length of the language ID list
...
// Function to get the length of a specific string descriptor
uint8_t USB_Host_Device_Manager___Get_String_Descriptor_Length(uint8_t port_Number, uint8_t device_Address, uint8_t string_Type)
{
    ...
    {
        // Return the minimum of the descriptor length and the maximum allowed length
...
// Function to get the total length of a configuration descriptor
...
// Function to parse and update the configuration descriptor information
void USB_Host_Device_Manager___Device_Update_Configuration_Descriptor(uint8_t port_Number, uint8_t device_Address, uint8_t configuration_Index)
{
    ...
    uint16_t i = 0;  // Index to traverse the configuration descriptor buffer
    ...
    while(i < total_Length)
    {
        ...
        case USB_Host___CONFIGURATION_DESCRIPTOR_TYPE:
            // Store pointer to the configuration descriptor
        ...
        case USB_Host___INTERFACE_DESCRIPTOR_TYPE:
            // Store pointer to the interface descriptor
        ...
        case USB_Host___ENDPOINT_DESCRIPTOR_TYPE:
            // Store pointer to the endpoint descriptor
        ...
        i += descriptor_Length;  // Move to the next descriptor
    }
}

// Function to get the configuration descriptor buffer
uint8_t* USB_Host_Device_Manager___Get_Configuration_Descriptor_Buffer(uint8_t port_Number, uint8_t device_Address, uint8_t configuration_Index)
{
    return(USB_Host_Device_Manager___Port[port_Number].p_Device[device_Address]->descriptor_Buffers.configuration_Descriptor_Buffer[configuration_Index]);
}

// Function to get the language ID descriptor buffer
...
// Function to get the manufacturer string descriptor buffer
...
// Function to get the product string descriptor buffer
...
// Function to get the serial number string descriptor buffer
...
// Function to update the length of a specific string based on the descriptor
void USB_Host_Device_Manager___Update_String_Length(uint8_t port_Number, uint8_t device_Address, uint8_t string_Type)
{
    ...
    if(string_Length != 0)
    {
        string_Length = (string_Length - 2) / 2;  // Calculate the actual length of the string (excluding descriptor header)
    }
...
// Function to get a language ID from the language ID list
...
// Function to get the device descriptor structure
...
// Function to set the maximum packet size for an IN endpoint
...
// Function to set the maximum packet size for an OUT endpoint
...
// Function to set the device configuration and update endpoint packet sizes
void USB_Host_Device_Manager___Set_Configuration(uint8_t port_Number, uint8_t device_Address, uint8_t configuration_Index)
{
    for(uint8_t i = 0; i < USB_Host_Device_Manager___Port[port_Number].p_Device[device_Address]->descriptors.configuration[configuration_Index].p_Configuration_Descriptor->bNumInterfaces; i++)
    {
        for(uint8_t j = 0; j < USB_Host_Device_Manager___Port[port_Number].p_Device[device_Address]->descriptors.configuration[configuration_Index].interface[i].p_Interface_Descriptor->bNumEndpoints; j++)
        {
            ...
            if(endpoint_Direction == 1)  // IN endpoint
            {
...
// Function to get the maximum packet size for an OUT endpoint
...
// Function to get the maximum packet size for an IN endpoint
...
// Function to get the current packet ID for an endpoint
...
// Function to set the current packet ID for an endpoint
...
// Function to check if a device is a low-speed device
...
// Function to remove a device from a port
void USB_Host_Device_Manager___Port_Remove_Device(uint8_t port_Number, uint8_t device_Address)
{
    ...
    USB_Host_Device_Manager___Free_Device(p_Device);  // Free the device structure
    USB_Host_Device_Manager___Port[port_Number].p_Device[device_Address] = NULL;  // Remove the device from the port
}

// Function to set whether a device is the root device
...
// Function to assign a device to a specific address on a port
...
// Function to find a free device address on a port
uint8_t USB_Host_Device_Manager___Port_Get_Free_Device_Address(uint8_t port_Number)
{
    ...
    return (0);  // Return 0 if no address is available
}

// Function to copy a device pointer from one address to another on a port
...
// Function to reserve a new device address on a port
...
// Function to change the current USB address of a device
...
// Function to clear the device connected/disconnected flag on a port
...
// Function to set the device connected/disconnected flag on a port
...
// Function to check if a device has been connected or disconnected on a port
...
void USB_Host_Device_Manager___Device_Disconnected(uint8_t port_Number, uint8_t device_Address)
{
    ...
    if(p_Device != NULL)
    {
        USB_Host_Device___Remove_Polling_Device(port_Number, device_Address);  // Remove device from polling list
        ...
        {
            // Call the disconnection callback if it's set
...
// Function to check the connection flag of a device
...
// Function to clear the connection flag of a device
...
// Function to set the port number in the device status
...
// Function to set the speed of a device
...
// Function to get the root device address of a port
...
// Function to initialize device descriptor buffers
...
// Function to handle start-of-frame events for polling devices
void USB_Host_Device_Manager___Handle_Start_Of_Frame(uint8_t port_Number)
{
    if(USB_Host_Device_Manager___Polling_List->first_Node != NULL)
    {
        ...
        while (p_Polling_Node != NULL)
        {
            ...
            {
                p_Polling_Node->polling_Counter = 0;
                p_Polling_Node->callback(port_Number, p_Polling_Node->device_Address);  // Invoke the callback function
            }
            p_Polling_Node = p_Polling_Node->next_Node;  // Move to the next node in the list
        }
    }
}

// Function to set up a device after allocation
...
// Function to allocate a device at address zero
...
// Function to enable a device (mark as connected)

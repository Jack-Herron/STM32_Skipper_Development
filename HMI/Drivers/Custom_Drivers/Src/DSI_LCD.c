...
void DSI_LCD___DSI_Init(void)
{
	...
	DSI -> LCOLCR   =   DSI_LCD___LCOLC_RGB565; 										// Set color coding format to RGB565
...
{
	GPIOD->MODER |= GPIO_MODER_MODER12_0; 												// Set PH7 to Output mode
	GPIOD->ODR &= ~GPIO_ODR_OD12; 														// Set PH7 Low (LCD Reset active)
	clock___delay_ms(20);					                                            // Delay 20ms
	GPIOD->ODR |= GPIO_ODR_OD12; 														// Set PH7 High (LCD Reset inactive)
...
void DSI_LCD___LTDC_Init(void)
{
	...
	LTDC_Layer1->CFBLR 	= 	((DSI_LCD___BYTES_PER_PIXEL * DSI_LCD___HACT) 			<< LTDC_LxCFBLR_CFBP_Pos) | 	// Set color frame buffer line pitch
							(((DSI_LCD___BYTES_PER_PIXEL * DSI_LCD___HACT) + 3) 	<< LTDC_LxCFBLR_CFBLL_Pos);		// Set color frame buffer line length
...
{
	RCC->AHB1ENR |= RCC_AHB1ENR_GPIODEN; 												// Enable GPIOH clock
